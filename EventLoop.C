...
void EventLoop::openFile()
{
  fileName = "/home/ap14092/Higgstest/csv2/abgLep.txt";
  ...
  events = 0;
  bg_count = 1;
  fout << " Event_Weight, Truth_Mass, Tags, MET_Pt, Lep_Eta, Lep_Pt, Higgs_Pt, Higgs_Mass, Wplus_Pt, Wplus_Mass, m_DeltaPhi_HW, m_DeltaPhi_LH, m_DeltaPhi_LW" << endl;

}

void EventLoop::closeFile()
{
  fout.close();
...
void EventLoop::analyseEvent()
{
    ...
    if (pass_sel["Merged_LepN_CR"] || pass_sel["Resolved_LepN_CR"] || pass_sel["Merged_LepP_CR"] || pass_sel["Resolved_LepP_CR"] || pass_sel["Merged_LepN_SR"] || pass_sel["Resolved_LepN_SR"] || pass_sel["Merged_LepP_SR"] || pass_sel["Resolved_LepP_SR"])
    {
        ...
        h_DeltaPhi_HW->Fill(m_DeltaPhi_HW, m_EventWeights, pass_sel, m_NTags);
      	h_DeltaPhi_LH->Fill(m_DeltaPhi_LH, m_EventWeights, pass_sel, m_NTags);
       	h_DeltaPhi_LW->Fill(m_DeltaPhi_LW, m_EventWeights, pass_sel, m_NTags);
        ...
        h_m_NTags_trkJ->Fill(m_NTags_trkJ, m_EventWeights, pass_sel, m_NTags);
       	...
       	//if statement for selecting merged lep p for signal region and lep negative
	/*if (pass_sel["Merged_LepP_SR"]){
	  events++; 
	   fout << m_EventWeights[0] << ",  " << m_MassTruth << ", " << m_NTags << ",  " << MET->Pt() * 0.001 << ",  " <<  Lepton4vector->Eta() << ",  " <<  Lepton4vector->Pt() << ",  " <<  Higgs.Pt() << ", " << Higgs.M() << ",  " <<  Wplus.Pt()  << ", " << Wplus.M() << ", " << m_DeltaPhi_HW << ", " << m_DeltaPhi_LH << ", " << m_DeltaPhi_LW <<  endl;
	   }*/
	
	if (pass_sel["Merged_LepP_SR"] && bg_count % 2 != 0){
	  fout << m_EventWeights[0] << ",  " << h_m800 << ", " << m_NTags << ",  " << MET->Pt() * 0.001 << ",  " <<  Lepton4vector->Eta() << ",  " <<  Lepton4vector->Pt() << ",  " <<  Higgs.Pt() << ", " << Higgs.M() << ",  " <<  Wplus.Pt()  << ", " << Wplus.M() << ", " << m_DeltaPhi_HW << ", " << m_DeltaPhi_LH << ", " << m_DeltaPhi_LW <<  endl;
	  bg_count += 1;
	}
	if (pass_sel["Merged_LepP_SR"] && bg_count % 2 == 0){
	  fout << m_EventWeights[0] << ",  " << h_m1600 << ", " << m_NTags << ",  " << MET->Pt() * 0.001 << ",  " <<  Lepton4vector->Eta() << ",  " <<  Lepton4vector->Pt() << ",  " <<  Higgs.Pt() << ", " << Higgs.M() << ",  " <<  Wplus.Pt()  << ", " << Wplus.M() << ", " << m_DeltaPhi_HW << ", " << m_DeltaPhi_LH << ", " << m_DeltaPhi_LW <<  endl;
	  bg_count += 1;
	}
...
{ //THIS FUNCTION RESTRICTS THE EVENT BAISED ON THE CHOSEN JET PARAMETERS
    ...
    //angle between Wplus and Higgs
    m_DeltaPhi_HW = fabs(Wplus.DeltaPhi(Higgs));
    m_DeltaPhi_LH = Higgs.DeltaR(*Lepton4vector);
    m_DeltaPhi_LW = Wplus.DeltaR(*Lepton4vector);
...
void EventLoop::Write(TDirectory *dir, std::string dirname)
{
    ...
    h_DeltaPhi_LH->Write(dir, ("DeltaPhi_LH"));
    h_DeltaPhi_LW->Write(dir, ("DeltaPhi_LW"));
    h_pTH->Write(dir, ("pTH"));
